...
class PongGame(QWidget):
    ...
    def update_game(self):
        ...
        if self.keys_pressed[Qt.Key.Key_S] and self.player1_y < self.height() - self.paddle_height:
            self.player1_y += self.paddle_speed
        if self.keys_pressed[Qt.Key.Key_Up] and self.player2_y > 0:
            self.player2_y -= self.paddle_speed
        if self.keys_pressed[Qt.Key.Key_Down] and self.player2_y < self.height() - self.paddle_height:
            self.player2_y += self.paddle_speed
        ...
        paddle1_rect = QRectF(20, self.player1_y, self.paddle_width, self.paddle_height)
        paddle2_rect = QRectF(self.width() - 20 - self.paddle_width, self.player2_y, 
                             self.paddle_width, self.paddle_height)
        ball_rect = QRectF(self.ball_x, self.ball_y, self.ball_size, self.ball_size)
        ...
        if ball_rect.intersects(paddle1_rect) and self.ball_speed_x < 0:
            ...
            self.ball_speed_x = min(self.ball_speed_x * 1.1, self.max_ball_speed)
            self.ball_speed_y = min(abs(self.ball_speed_y * 1.1), self.max_ball_speed) * (1 if self.ball_speed_y > 0 else -1)
            
        if ball_rect.intersects(paddle2_rect) and self.ball_speed_x > 0:
            self.ball_speed_x = -self.ball_speed_x
            # Увеличиваем скорость после отскока для усложнения игры
            self.ball_speed_x = max(self.ball_speed_x * 1.1, -self.max_ball_speed)
            self.ball_speed_y = min(abs(self.ball_speed_y * 1.1), self.max_ball_speed) * (1 if self.ball_speed_y > 0 else -1)
    ...
    def paintEvent(self, event):
        ...
        # painter.drawText(int(self.width() / 2 - 200), 30, 
        #                  "Управление: Игрок 1: W/S | Игрок 2: Стрелки Вверх/Вниз")
        ...
        painter.fillRect(20, int(self.player1_y), self.paddle_width, self.paddle_height, QColor(255, 255, 255))
        painter.fillRect(self.width() - 20 - self.paddle_width, int(self.player2_y), 
                        self.paddle_width, self.paddle_height, QColor(255, 255, 255))
        painter.fillRect(int(self.ball_x), int(self.ball_y), self.ball_size, self.ball_size, QColor(255, 255, 255))
        ...
        if self.score1 >= 5 or self.score2 >= 5:
            ...
            painter.drawText(int(self.width() / 2 - 100), int(self.height() / 2), f"{winner} победил!")
            painter.drawText(int(self.width() / 2 - 150), int(self.height() / 2 + 40), "Нажмите R для рестарта")
