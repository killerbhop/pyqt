...
class PuzzlePiece(QLabel):
    ...
    def dropEvent(self, event):
        ...
        if isinstance(source, PuzzlePiece):
            ...
            source_pos = source_parent.layout().indexOf(source)
            target_pos = target_parent.layout().indexOf(self)
...
class CaptchaWidget(QWidget):
    ...
    def initUI(self, image_paths):
        ...
        self.target_widget.setFixedSize(self.max_piece_size * 2 + 10, self.max_piece_size * 2 + 10)
        ...
        for i, path in enumerate(image_paths):
            ...
            if pixmap.isNull():
                ...
                pixmap = pixmap.scaled(self.max_piece_size, self.max_piece_size, 
        ...
        for i, piece in enumerate(self.pieces):
            self.source_layout.addWidget(piece, i, 0, alignment=Qt.AlignmentFlag.AlignCenter)

        self.target_labels = []
        for i in range(4):
            placeholder = QLabel(self)
            placeholder.setFixedSize(self.max_piece_size, self.max_piece_size)
            placeholder.setStyleSheet("border: 2px dashed gray; background-color: #f0f0f0;")
            placeholder.setAcceptDrops(True)
            placeholder.dropEvent = lambda event, idx=i: self.handle_target_drop(event, idx)
            placeholder.dragEnterEvent = lambda event: event.acceptProposedAction()
            self.target_labels.append(placeholder)
            row, col = i // 2, i % 2
            self.target_layout.addWidget(placeholder, row, col, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        container_layout.addWidget(buttons_widget, alignment=Qt.AlignmentFlag.AlignCenter)
            ...
            if target_label.pixmap() and not target_label.pixmap().isNull():
                for piece in self.pieces:
                    if piece.pixmap().toImage() == target_label.pixmap().toImage():
                        piece.show()
                        self.source_layout.addWidget(piece, self.source_layout.count(), 0, 
    ...
    def check_completion(self):
        ...
        if self.is_completed:
            ...
            QMessageBox.warning(self, "Ошибка", "Капча решена неправильно! Попробуйте еще раз.")
            self.reset_puzzle()

    def reset_puzzle(self):
        for label in self.target_labels:
            label.clear()
            label.setStyleSheet("border: 2px dashed gray; background-color: #f0f0f0;")
...
class AuthWindow(QWidget):
    ...
    def handle_login(self):
        ...
        if login_attempts >= self.max_attempts:
            QMessageBox.warning(self, "Превышено количество попыток", 
                              "Вы превысили количество попыток входа. Требуется проверка капчи.")
    ...
    def handle_register(self):
        ...
        if self.db.register_user(name, email, password):
            ...
            self.start_game()  # Запускаем игру после регистрации
        else:
